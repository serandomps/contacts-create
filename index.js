...
dust.loadSource(dust.compile(require('./template'), 'contacts-create'));
...
var render = function (ctx, container, data, done) {
    ...
    dust.render('contacts-create', data, function (err, out) {
        if (err) {
            return done(err);
        }
        var elem = sandbox.append(out);
        var contactsForm = form.create(container.id, elem, configs);
        contactsForm.render(ctx, data, function (err) {
            if (err) {
                return done(err);
            }
